...
def plot_llr_individual(config):
    ...
    for i in range(int(f"{config['limits']['nestimators']}")):
      ...
      if config['limits']['method'] == 'alices':
         title = r'$\bf{ALICE}$' + r'$\ (\alpha = 0, n_{\theta_0} = 10000)$'
      ...
      plt.text(-0.02, 1.0, '68% CL', color='gray', verticalalignment='bottom', horizontalalignment='left', fontsize=11)
      plt.text(-0.02, 3.84, '95% CL', color='gray', verticalalignment='bottom', horizontalalignment='left', fontsize=11)
...
config = {
    ...
    'limits': {
        ...
        'model': 'alices_hidden_[50]_tanh_alpha_0_epochs_50_bs_128',
        'method': 'alices',
        'lumi': 300,
        'grid_ranges': [-0.2, 0.2],
