...
# Get event data (observations and weights)
    ...
    plt.text(
    x_position, y_position, luminosity_info,
    fontsize=8,
...
if __name__ == '__main__':
    ...
    obs_xlabel_dict={
        ...
        'dphi_ll':r'$\Delta \phi_{ll}$',
        'pt_l':r'$p_T^\ell ~ [GeV]$',
        'mt_tot':r'$m_T^{\mathrm{tot}}\ (GeV)$',
        'm_jj':r'$m_{jj}\ (GeV)$',
        'pz_nu':r'$p_z^{\nu}$',
        'pt_tot':r'$p_T^{\mathrm{tot}}\ (GeV)$',
    ...
    if not args.plot_sally:
        ...
        histo_observables=plot_distributions_split_backgrounds(filename=f'{args.main_dir}/{args.sample_type}.h5',
        parameter_points=['sm',np.array([1.5]),np.array([-1.5])] if args.sample_type!={args.background_sample} else [],
        #change here the desired points to plot:
        filename_bkgonly=f'{args.main_dir}/{args.background_sample}.h5',
        ...
        line_labels=['SM',r'$c_{H\tilde{G}} = 1.5$',r'$c_{H\tilde{G}} = -1.5$'] if args.sample_type!={args.background_sample} else [],
        log=args.do_log,
        ...
        histo_observables.savefig(f'{args.plot_dir}/{args.sample_type}{plot_stem}.pdf', dpi=600,bbox_inches='tight')
        
        ###### Plot distributions with pannel ratio
        histo_observables=plot_distribution_with_ratio(filename=f'{args.main_dir}/{args.sample_type}.h5',
        parameter_points=['sm',np.array([0.005]),np.array([-0.005])] if args.sample_type!={args.background_sample} else [],
        #parameter_points=['sm',np.array([1.150]),np.array([-1.035]),] if args.sample_type!={args.background_sample} else [],
        filename_bkgonly=f'{args.main_dir}/{args.background_sample}.h5',
        observables=args.observables,
        observable_labels=[obs_xlabel_dict[obs] if obs in obs_xlabel_dict else obs for obs in args.observables] if args.observables!= None else None,
        line_labels=['Signal Only',r'$(c_{H\tilde{G}}) = (0.005)$',r'$(c_{H\tilde{G}}) = (-0.005)$'] if args.sample_type!={args.background_sample} else [],
        log=args.do_log,
        ...
        # histo_observables.savefig(f'{args.plot_dir}/{args.sample_type}{plot_stem}_number_of_events_with_ratio_c_hg_defaultvalue.pdf', dpi=600,bbox_inches='tight')
    

    else:
        
        histo_sally=plot_sally_distributions_split_backgrounds(filename=f'{args.main_dir}/{args.sample_type}.h5',
        parameter_points=['sm',np.array([1.5]),np.array([-1.5]),] if args.sample_type!={args.background_sample} else [],
        filename_bkgonly=f'{args.main_dir}/{args.background_sample}.h5',
        model_path=f'{args.main_dir}/models/sally/{args.sally_observables}/{args.sally_model}/sally_ensemble_{args.sally_type}',
        line_labels=['SM + Backgrounds ',r'$c_{H\tilde{G}} =1.5$',r'$c_{H\tilde{G}} = -1.5$'] if args.sample_type!={args.background_sample} else [],
        linestyles=None if args.sample_type!={args.background_sample} else ['dashdot'],
        ...
        histo_sally.savefig(f'{args.plot_dir}/sally_{args.sample_type}_{args.sally_observables}_{args.sally_model}_{args.sally_type}{plot_stem}.pdf')